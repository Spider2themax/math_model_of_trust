...
class TrustSimulator:
    ...
    def simulate_society(
        ...
        for iteration in range(iterations):
            ...
            trust_updates = []
            laws = institution._generate_laws(
                society=society, mean=mean, stdev=stdev, law_type=law_type
            )
            for person_id in range(society.population_size):
                trust_updates.append(
                    self._calculate_trust_update(
                        society=society,
            ...
            society = self._update_societal_trust(
                society=society, trust_updates=trust_updates
            )
            # Save current overall societal trust.
            societal_trust.append(
                np.mean(
                    [
                        society.person_vector[person_id].get_trust()
                        for person_id in range(society.population_size)
                    ]
                )
            )
            ...
            trust_updates = []
            # Update edge matrix
            society = self._update_edge_matrix(
                society=society, affiliation_prob=affiliation_prob
            )
    ...
    def _calculate_trust_update(
        self, society, institution, person_id, laws, K=1, J=1, alpha=1
    ):
        """
        ...
        for j in range(society.population_size):
            if person_id != j:
                ...
                update = update + K * society.edge_matrix[person_id][j] * np.tanh(
                    alpha * society.person_vector[j].get_trust()
                )
        # Have the laws positively update those who share the affiliation of the
        # institution, and negatively affect the opposite.
        for law in laws:
            update = update + J * law.affected_persons[person_id] * np.tanh(
                alpha * law.actual_value
            )
    ...
    def _update_societal_trust(self, society, trust_updates):
        for person_id in range(society.population_size):
            society.person_vector[person_id].update_trust(
                update=trust_updates[person_id]
            )
        return society
        ...
        for person_id_first in range(society.population_size):
            for person_id_second in range(society.population_size):
                if person_id_first == person_id_second:
                    ...
                    party_delta = np.tanh(
                        society.person_vector[person_id_first]._party_affiliation
                    ) - np.tanh(
                        society.person_vector[person_id_second]._party_affiliation
                    )
                    ...
                    adjusted_connectivity_probability = (
                        1 - (party_delta) / 2
                    ) * affiliation_prob
    ...
    def _update_society_party(self, society, J=1, alpha=1):
        ...
        update = 0
        for person_id in range(society.population_size):
            society.person_vector[person_id].update_party_affiliation(
                self._calculate_affiliation_update(person_id, J, society, alpha)
            )
        return society
    ...
    def _calculate_affiliation_update(self, person_id, K, society, alpha):
        ...
        for person_id_new in range(society.population_size):
            if person_id != person_id_new:
                update = update + K * society.edge_matrix[person_id][
                    person_id_new
                ] * np.tanh(
                    alpha * society.person_vector[person_id_new].get_affiliation()
                )
